...
export async function closeJobListing(
  ...
  try {
    // Use the dedicated close endpoint — DELETE /api/v1/jobs/{id}/close
    const closeResponse = await fetch(`${backendBaseUrl}/api/v1/jobs/${jobId}/close`, {
      method: "DELETE",
    });

    if (!closeResponse.ok) {
      const errorBody = await closeResponse.json().catch(() => ({}));
      return {
        status: "error",
        message: errorBody?.detail ?? `Failed to close listing (${closeResponse.status}).`,
      };
    }

    // Fetch the updated job to get the title for the success message
    const jobResponse = await fetch(`${backendBaseUrl}/api/v1/jobs/${jobId}`, {
      cache: "no-store",
    });
